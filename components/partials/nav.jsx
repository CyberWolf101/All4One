import React, { useContext } from 'react';
import { Link, useNavigate } from 'react-router-dom';
import Logo from '../../Assets/logo1.png'
import Logo2 from '../../Assets/logo2.png'
import { Avatar, Spinner, useDisclosure } from '@chakra-ui/react';
import useAuth from '../../hooks/auth';
import { AccountBalanceOutlined, Diversity1Outlined, Menu, MoreVert, NotificationsOutlined, Person2Outlined, PointOfSale, PointOfSaleOutlined, WalletOutlined } from '@mui/icons-material';
import { useState } from 'react';
import { useEffect } from 'react';
import { useAuthState } from 'react-firebase-hooks/auth';
import { auth, db } from '../../config';
import { useLogout } from "../../hooks/logout";
import { doc, getDoc } from 'firebase/firestore';
import { userContext } from '../../contexts/userContext';
import { GeneralNotifications } from '../../contexts/generalNotificationContext';
import { Fade } from 'react-reveal';

function Nav(props) {
    const settingsID = 'XA5eUSx0KyLCndY3TiDl';
    // const { user } = useAuth()
    const [unReadNotifications, setunReadNotifications] = useState(0)
    let filterNotifications = []
    const [authUser, isLoading, error] = useAuthState(auth)
    const { logout, isLoading: loggingOut } = useLogout()
    const nav = useNavigate()
    const [userDetails, setuserDetails] = useContext(userContext)
    const [generalNotifications, setGeneralNotifications] = useContext(GeneralNotifications)

    const handleAuth = () => {
        if (userDetails?._id) {
            logout()
        }
        if (!userDetails || !userDetails?._id) {
            nav('/auth/user')
        }

    }
    //info
    //logout

    return (
        <div className=''>
            <Fade top duration={1500}>
                <div className='mb-1 px-2 py-3 grid-nav shadow-sm bg-success fixed-top'>
                    <div className='text-white menu-div' type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasExample" aria-controls="offcanvasExample">
                        <Menu />
                    </div>
                    <div className='text-white straight flex shi' style={{ width: '90px', fontSize: '15px', flexDirection: 'column', letterSpacing: '1.5px' }}>
                        INDEED
                        {/* OSHOFREE */}
                        <div style={{ fontSize: '6px' }}>All4one</div>
                        {/* <img src={Logo2} alt="OSHOFREE" /> */}
                        {/* <div className='tiny' >all 4 one</div> */}
                        {/* <img src='data:image/png;base64,' alt="Indeed" className='logo' /> */}
                    </div>

                    <div className="flex " style={{ justifyContent: 'flex-end' }}>
                        <button className="btn btn-sm btn-outline-light me-1" onClick={handleAuth}>
                            <MoreVert fontSize='small'/>
                            {/* {userDetails?._id ? 'Logout' : 'Login'} */}
                            
                            </button>
                        <div className="shopBag-div">
                            <div className="absolute_div">
                                {/* <div className="badge_number bg-white rounded-pill">{unReadNotifications > 0 ? unReadNotifications : 0}</div> */}
                                <div className="badge_number bg-white rounded-pill">{userDetails?.unread ? userDetails?.unread : 0}</div>
                            </div>
                            <div className="shopBag" onClick={() => nav(`/user/profile/${userDetails?._id}`)}>
                                <Person2Outlined fontSize="inherit" />
                            </div>
                        </div>
                    </div>
                </div>
            </Fade>


            {/* _________OFFCANVAS____________- */}
            <div className="offcanvas offcanvas-start" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel" style={{ width: '70%' }}>
                <div className="offcanvas-header">
                    <div>
                        {/* <img src={Logo} alt="Oshofree" style={{ height: '20px' }} /> */}
                        <img src='data:image/png;base64,' alt="Indeed" className='logo' />
                    </div>
                    <button type="button" className="btn-close" data-bs-dismiss="offcanvas" aria-label="Close"></button>
                </div>
                <hr />
                <div className="offcanvas-body">


                    <div className='text-success'>
                        <div className='flex my-1' onClick={() => nav(`/user-transactions/${userDetails.id}`)} data-bs-dismiss="offcanvas">
                            <div>-My Transactions</div> <PointOfSaleOutlined fontSize='small' />
                        </div>
                        <hr />
                        <div className='flex mt-2' onClick={() => nav(`/interact-users/${userDetails.id}`)} data-bs-dismiss="offcanvas">
                            <div>-Find Users</div> &nbsp; <Diversity1Outlined fontSize='small' />
                        </div>
                    </div>
                    <hr />
                    <div>
                        <div className="border straight py-2 mt-5" style={{ flexDirection: 'column' }}>
                            <div className="small faint">
                                Balance <AccountBalanceOutlined fontSize='inherit' />
                            </div>
                            <div className="text-success">
                                ₦ {userDetails?.bal ? Number(userDetails?.bal).toLocaleString() : '0.00'}
                            </div>
                        </div>

                        <div className='flex straight mt-3'>
                            <button
                                className='btn btn-outline-success mx-1 btn-sm'
                                onClick={() => nav(`/withdraw-amount`)}
                                data-bs-dismiss="offcanvas"
                            >
                                Withdraw
                            </button>
                            <button
                                onClick={() => nav(`/deposit-amount`)}
                                className='btn btn-success mx-1 btn-sm'
                                data-bs-dismiss="offcanvas"
                            >
                                Deposit
                            </button>
                        </div>
                        {/* <hr /> */}
                        {/* <div className='flex_normal px-2' style={{ flexDirection: 'row-reverse' }}>
                            <div className="mt-5 text-success straight" >
                                <Link
                                    to={`/user-transactions/${userDetails.id}`}
                                    className='my-1 btn btn-sm btn-success'
                                    style={{ width: '100px' }}
                                >  <small>Transactions</small></Link>
                            </div>
                            <div className="mt-5 text-success straight" >
                                <Link
                                    to={`/interact-users/${userDetails.id}`}
                                    className='my-1 btn btn-sm btn-outline-success'
                                    style={{ width: '100px' }}
                                >
                                    <small>Interact</small>
                                </Link>
                            </div>
                        </div> */}
                        <hr />
                    </div>
                </div>
            </div>

            <br />
            <br />
            <br />
        </div>
    );
}

export default Nav;